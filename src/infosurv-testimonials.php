...
$_ENV['mode'] = 'dev';
define('PLUGIN_ROOT',  plugins_url(null, dirname(__FILE__)).DIRECTORY_SEPARATOR);
define('PLUGIN_DIR',   PLUGIN_ROOT.'src'.DIRECTORY_SEPARATOR);
define('PLUGIN_VIEWS', PLUGIN_DIR.'views'.DIRECTORY_SEPARATOR);
...
class InfTestimonials 
{
	...
	public $source;
	public $link;
	public $current_post_classnames;
	...
	public function attatchActionsAndFilters()
	{
		...
		add_action( 'init', array($this, 'testimonials_init'), 10);	
	...
	{	
		...
		wp_register_script('isotope', PLUGIN_JS.'isotope.js', array(), null, true); 
		wp_register_script('global', 	PLUGIN_JS.'global.js', array(), null, true);
		
		wp_register_style('isotope',  PLUGIN_CSS.'isotope.css', array('styles'), null, 'all'); //Take out the styles dependency for future work
		wp_register_style('styles2',  PLUGIN_CSS.'styles2.css', array(), null, 'all');
		
		if(! is_admin() && is_page(4625)){
			//echo plugins_url(null, dirname(__FILE__)).DIRECTORY_SEPARATOR.'src'.DIRECTORY_SEPARATOR.'functions.php';
	...
	public function attatchCategories()
	{
		global $post;
		$classname = "";
		$id = $post->ID;
		...
		{
			$classname .=  $cat->category_nicename." ";
		}
